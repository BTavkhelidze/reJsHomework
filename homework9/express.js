...
import isKeyMiddleware from './middleware/expenses.middleware.js';
...
app.post('/expenses', async (req, res) => {
  ...
  if (!title || !amount || !category)
    return res
      .status(400)
      .json({ message: 'please enter a title, amount and category' });
...
app.delete('/expenses/:id', isKeyMiddleware, async (req, res) => {
  const expenses = await readFile('expenses.json', true);
  const id = Number(req.params.id);
  const expenseIndex = expenses.findIndex((element) => element.id === id);
  // console.log(expenses[expenseIndex]);
  const deletedExpense = expenses.splice(expenseIndex, 1);

  // // if (expenseIndex === -1) return res.status(400).json('could not fount id ');
  await writeFile('expenses.json', expenses, true);
  res
    .status(200)
